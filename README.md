...
| AgentDream | hybridservice | AI 梦境系统 — 自动记忆回顾、联想式整理、日记合并/删除/感悟审批 |
...
| EmojiListGenerator | static | 表情包清单生成器（启动时扫 `image/XXX表情包/` 生成 `{{XXX表情包}}` 占位符） |
| ImageServer | service | 图床服务（密码保护的 `/pw=<Image_Key>/images/*` 路径；与 EmojiListGenerator 配套为表情包提供 URL） |
