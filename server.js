...
async function initialize() {
    ...
    try {
        ...
        await Promise.all(txtFiles.map(async (fileName) => {
            ...
            try {
                cachedEmojiLists.set(emojiName, await fs.readFile(filePath, 'utf-8'));
            } catch (_) {}
        }));
        if (txtFiles.length > 0 && DEBUG_MODE) console.log(`[initialize] Loaded ${txtFiles.length} emoji lists.`);
    } catch (_) {
        // EmojiListGenerator plugin not installed, skip silently
    }
