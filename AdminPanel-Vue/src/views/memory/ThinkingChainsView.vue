<template>
  <div class="page">
    <PageHeader title="思维链编辑" subtitle="AI 元自学习产物 — 思维链簇" icon="psychology">
      <template #actions>
...
import { getThinkingChains, saveThinkingChains } from '@/api/rag'
...
async function reload() {
  loading.value = true
  try {
    const data = await getThinkingChains()
...
async function save() {
  try {
    ...
    await saveThinkingChains(parsed)
    original.value = json.value
