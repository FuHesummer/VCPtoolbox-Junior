...
class ContextFoldingV2 {
    constructor() {
        ...
        this.summaryQueue = [];          // 摘要任务队列（节流用）
        this.activeSummaryCount = 0;     // 当前正在执行的摘要任务数
    }
    ...
    async initialize(config, dependencies) {
        ...
        this.enabled = true;
        console.log(`[ContextFoldingV2] 初始化完成 (模型: ${this.summaryModel}, 最低深度: ${this.minDepth}, 最大并发: ${this.maxConcurrentSummaries}, 阈值基准: ${this.hotParams.thresholdBase}, Store: ${stats.count}/${stats.maxEntries}条)`);
    }
    ...
    /**
     * 消费队列：活跃数小于上限就出队执行，避免并发打爆上游 API
     */
