/**
 * VCP ServerFileOperator — AI 受限目录文件操作插件
 *
 * 配置来源（优先级高 → 低）：
 *   1. Plugin.js spawn 时注入的 env（读自 VCPtoolbox-Junior/config.env 全局区块）
 *   2. 本文件同目录 config.env（已弃用；保留 dotenv.config 仅为独立测试兜底）
 *   3. 代码硬编码默认值
 *
 * 全部配置字段说明参见 VCPtoolbox-Junior/config.env 的
 * [插件] VCP 服务器文件操作器 (FileOperator / ServerFileOperator) 区块。
 *
 * 关键安全设计：
 *   - ALLOWED_DIRECTORIES 为空 → 允许所有目录（仅限开发调试）
 *   - ALLOWED_DIRECTORIES 非空 → 白名单内全权限；白名单外只允许 ReadFile / FileInfo
 *   - 详见 isPathAllowed() 实现
 */
...
// 独立测试兜底：如插件目录仍有遗留 config.env 则加载；生产环境靠 Plugin.js 注入 env
require('dotenv').config({ path: path.join(__dirname, 'config.env') });

// ---- 配置读取（所有字段优先由全局 config.env 驱动） ----

// CANVAS_DIRECTORY: CreateCanvas 命令写入的默认目录（非用户配置字段，内部使用）
const CANVAS_DIRECTORY = process.env.CANVAS_DIRECTORY || path.join(__dirname, '..', '..', 'AppData', 'Canvas');

// ALLOWED_DIRECTORIES: 允许 AI 操作的目录白名单（绝对路径，逗号分隔）
// 留空 = 允许所有目录；配置后白名单外仅放行 ReadFile/FileInfo
const ALLOWED_DIRECTORIES = (process.env.ALLOWED_DIRECTORIES || '')
  .split(',')
  .map(dir => dir.trim())
  .filter(dir => dir);

// MAX_FILE_SIZE: 单次读/写/下载的字节上限（默认 20MB）
const MAX_FILE_SIZE = parseInt(process.env.MAX_FILE_SIZE) || 20971520;
// MAX_DIRECTORY_ITEMS: ListDirectory 返回的最大项数（防止列大目录爆上下文）
const MAX_DIRECTORY_ITEMS = parseInt(process.env.MAX_DIRECTORY_ITEMS) || 1000;
// MAX_SEARCH_RESULTS: SearchFiles 递归搜索返回的最大结果数
const MAX_SEARCH_RESULTS = parseInt(process.env.MAX_SEARCH_RESULTS) || 100;
// DEBUG_MODE: 插件专属调试日志（输出权限判定等内部行为，独立于全局 DebugMode）
const DEBUG_MODE = process.env.DEBUG_MODE === 'true';
// ENABLE_RECURSIVE_OPERATIONS: SearchFiles 是否递归遍历子目录（false 只搜一层）
const ENABLE_RECURSIVE_OPERATIONS = process.env.ENABLE_RECURSIVE_OPERATIONS !== 'false';
// ENABLE_HIDDEN_FILES: ListDirectory/SearchFiles 是否默认包含隐藏文件（以 . 开头）
