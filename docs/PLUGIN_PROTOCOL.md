...
### tvsVariables（TVS 工具指南协议，Junior v2.1+）

插件可自带"工具使用指南 txt"（类似 MCP 工具描述），通过 `tvsVariables` 让 Junior 自动管理：
    ...
    "tvsVariables": [
      {
        ...
        "description": "我的插件工具使用指南（AdminPanel 显示用）"
      }
    ]
  }
}
...
**目录约定**：插件目录下放 `tvs/myplugin.txt`，内容是给 AI 看的工具使用说明。

**字段约束**：
- `key`：必须以 `Var` 开头（如 `VarDreamTool`、`VarFileTool`）
- `file`：必须以 `.txt` 结尾，相对于插件根目录
- `description`：可选，AdminPanel / TVS 编辑器展示用

**生命周期**（由 [Plugin.js](../Plugin.js) `_registerPluginTvsVariables` / `_unregisterPluginTvsVariables` 实现）：

| 场景 | TVStxt/ 文件 | 插件 tvs/*.txt | process.env |
|------|-------------|---------------|-------------|
| **首次安装 / 启动**（TVStxt 无文件）| 新建（move 自插件，插件目录文件消失）| 被移走 | 注入 |
| **重启 / reload**（TVStxt 已有）| 保留（用户在 TvsEditor 的改动不丢）| 仍在插件里（从未 move 过）| 重注入 |
| **卸载（通过 PluginStore）** | 删除 | 随插件目录整体删除 | 清理 |

**"首次 move"设计理由**：TVStxt/ 是唯一真相，用户通过 AdminPanel TvsEditor 编辑不会被下次启动覆盖。插件目录下 `tvs/` 的文件仅作首次种子。

**禁忌**：
- ❌ 不要在 `config.env` 里重复声明协议化的 `Var*`（会与插件注册冲突）
- ❌ 同名 `Var` 被多个插件声明 → 启动 WARN 跳过
