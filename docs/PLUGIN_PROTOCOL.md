...
- 嵌入到仪表盘网格的 `.status-card.plugin-dashboard-card` 容器里
- **片段是 HTML 片段**（不是完整 `<html>`），直接在父页面 DOM 中执行，**可读 cookies / 调用 `/admin_api`**
...
**加载机制**：
- AdminPanel 启动时扫描插件 manifest，为每个声明 `adminNav` 的插件：
  - 在侧边栏（"插件管理"之后）注入 `<li>` 导航项
  - 创建隐藏 `<section>` 容器
- 用户点击导航时，`GET /admin_api/plugins/<name>/admin-page` 拉取 `admin/index.html`，提取 `<body>` 内容**内联**渲染到 section（不用 iframe）
- 内联渲染意味着：插件 admin 页面的 JS 直接跑在 AdminPanel 主 context，可复用主面板的 CSS 变量、API helper

**开关**：同 `dashboardCards`，用户可独立关闭。关闭时回退为默认的"管理弹窗"模式。

### admin/index.html 的两种渲染模式

同一个 `admin/index.html` 文件根据 manifest 配置，可以被 AdminPanel 用两种方式加载：

| 维度 | 弹窗模式（默认 / 回退） | 内联模式（`adminNav` 开启） |
|------|------------------------|----------------------------|
| 触发条件 | `manifest.adminNav` 未声明，或用户在 UI 偏好里关闭 | 声明了 `adminNav` 且用户 UI 偏好启用 |
| 入口 | 插件管理卡片的"管理"按钮 | 侧边栏独立导航项 |
| 渲染方式 | `<iframe src="/admin_api/plugins/<name>/admin-page">` | `fetch → 提取 <body> → 注入 <section>` |
| DOM 隔离 | 完全隔离 | 共享主面板 DOM/CSS/JS context |
| 可用 API | fetch `/admin_api/*`（同源）、postMessage（父子通信） | 所有主面板的 API + 全局工具（`apiFetch`, `showMessage` 等） |
| 样式影响 | 独立 iframe，不影响主面板 | 共享 CSS 变量；需避免 id 冲突 |
| 尺寸限制 | 约 80vw × 75vh（模态框）| 全屏 section，自由 |
| 适用场景 | 独立工具（如表单/图表/编辑器）| 高度嵌入的管理页面（如 DailyHot 的源头管理）|

**写页面时的注意点**：

```html
<!-- admin/index.html 应同时兼容两种模式 -->
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <style>
        /* 内联模式下这些样式会注入主页面，建议用唯一前缀 id 或类名 */
        .my-plugin-root { /* ... */ }
    </style>
</head>
<body>
    <!-- 内联模式只会提取 body 内容，所以所有内容放这里 -->
    <div class="my-plugin-root">
        <!-- 你的内容 -->
    </div>
    <script>
        // ⚠ 不要把变量挂在 window 上（可能与主面板冲突）
        // ✓ 用 IIFE 隔离作用域
        (function() {
            // 内联模式下 fetch 可以直接用相对路径
